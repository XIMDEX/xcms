...
<head>
    ...
    <link href='http://fonts.googleapis.com/css?family=Source+Sans+Pro:300,400,600,700,900,400italic,700italic'
                    ...
                    <li class="icon language-icon">
                        ...
                        <ul class="selector_language">
                            {section name=i loop=$locales}
                                <li {if ($user_locale.ID == $locales[i].ID || ( null == $user_locale && $locales[i].ID == $smarty.const.DEFAULT_LOCALE)  )} class="selected icon" {else} ng-click="changeLang('{$locales[i].Code}','{$locales[i].Name|gettext} ({$locales[i].Lang})')" {/if}>{$locales[i].Name|gettext}
                                    ({$locales[i].Lang})
                                </li>
                            {/section}
                        </ul>
