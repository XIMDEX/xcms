...
fn(function() {
    ...
    var url=fn(".depUrl").val();
    idNode=fn(".idNode").val();
    d3.json(url, function(data) {
        ...
        var masterTypes=[];
        var childTypes=[];
        var centerType,elemento,numMasters=0,numChildren=0,numCenters=0;
        for(var n in data.data){
            elemento = data.data[n];
            if(elemento.position=="master"){
                numMasters++;
            }else if(elemento.position=="child"){
                numChildren++;
            }else if(elemento.position=="center"){
                numCenters++;
            }
            if(masterTypes.indexOf(elemento.type)<0 && childTypes.indexOf(elemento.type)<0 && centerType!=elemento.type){
                if(elemento.position=="master"){
                    masterTypes.push(elemento.type);
                }else if(elemento.position=="child"){
                    childTypes.push(elemento.type);
                }else if(elemento.position=="center"){
                    centerType=elemento.type;
                }
            }
        }
        var xpos,ypos,model;
        ...
        for(var m in masterTypes){
            if(masterTypes.hasOwnProperty(m)){
                ...
                config.constraints.push(model);
                degree=degree+180/(masterTypes.length+1);
            }
        }
        ...
        for(var c in childTypes){
            if(childTypes.hasOwnProperty(c)){
                ...
                config.constraints.push(model);
                degree=degree-180/(childTypes.length+1);
            }
        }
...
function drawGraph() {
    ...
    var display = fn('.graph').css('display');
    fn('.graph')
        .css('display', 'block')
        .css('height', config.graph.height + 'px');
    graph.width  = fn('.graph').width()  - graph.margin.left - graph.margin.right;
    graph.height = fn('.graph').height() - graph.margin.top  - graph.margin.bottom;
    fn('.graph').css('display', display);
    ...
    graph.svg = d3.select('#graph'+idNode).append('svg')
        .attr('width' , graph.width  + graph.margin.left + graph.margin.right)
        .attr('height', graph.height + graph.margin.top  + graph.margin.bottom)
        .append('g')
        .attr('transform', 'translate(' + graph.margin.left + ',' + config.graph.margin.top + ')');
    ...
    fn('.graph-container').on('scroll', function() {
        graph.legend.attr('transform', 'translate(0,' + fn(this).scrollTop() + ')');
    });
    ...
    graph.line = graph.svg.append('g').selectAll('.link')
      ...
      .enter().append('line')
        .attr('class', 'link')
        .attr("marker-end", "url(#end"+idNode+")");
    ...
    setTimeout(function() {
       ...
       for (var i = 0; i < config.graph.ticksWithoutCollisions; i++) {
        ...
        graph.force.stop();
        graph.preventCollisions = true;
        fn('.graph-container').css('visibility', 'visible');
    });
}
        ...
        quadtree.visit(function(quad, x1, y1, x2, y2) {
            if (quad.point && quad.point !== obj) {
                ...
                if (ix) {
                    ...
                    if (adj == ya1) {
                        if(obj.y>=adj/2 /*&& (p.y+adj/2)>graph.height*/){
                            obj.y -= adj / 2;
                            p.y   += adj / 2;
                        }else{
                           adj = Math.min(xa1, xa2, ya2); 
                        }
                    } else if (adj == ya2) {
                        if(p.y>=adj/2  /*&& (obj.y+adj/2)>graph.height*/){
                            obj.y += adj / 2; 
                            p.y   -= adj / 2;
                        }else{
                           adj = Math.min(xa1, xa2); 
                        }
                    }
                    if (adj == xa1) {
                        obj.x -= adj / 2;
...
var docsClosePadding  = 8,
    desiredDocsHeight = 300;

function resize(showDocs) {
    $graph = fn('.graph-container');
    $graph.css('height', config.graph.height + 'px');
}
});