...
<h2>{t}Add group{/t}</h2>
	...
	{button
		...
		class="deletegroupuser validate btn"
	...
	<div class="action_content"><fieldset>
		...
		<fieldset class="buttons-form">{button label="Add group" title="Add group" class="addgroupuser validate  btn main_action"}<!--message="You are adding a new subscription. Would you like to continue?"-->
									</fieldset>
			{else}
					<p>{t}There are not{/t} {if ($user_groups_with_role)}{t}more{/t}{/if} {t}available groups to be associated with the user{/t}</p>
			{/if}
			</fieldset>
			<h2>{t}Remove from group{/t}</h2>
		<fieldset>
