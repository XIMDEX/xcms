...
<form method="post" name="publication_form" id="publication_form" action="{$action_url}">
    ...
    <ol class="numbered">
     ...
     <p>   {t}Would you like to publish just {/t}{$node_name}{t} or also contained subsections?{/t}</p><!-- <p>   {t}Would you like to publish just {if $folderName == 'sección'} this {else} this {/if} {$folderName} or contained subsections in {if $folderName == 'sección'} also, ella{else} él{/if}?{/t}</p> -->
