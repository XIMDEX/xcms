...
function setInfo(path, ID) {
	...
	document.getElementById('previewImg').innerHTML = '<input type="Text"  class="cajag" NAME="previewImgSrc" id="previewImgSrc" value="' + path + '" BORDER=0 ALT="vista previa">';
	}
...
function useFile()
{
	...
	var arr = new Array();
  	arr["ID"] = ID_imagen;
	// antes: arr["ID"] = "@@@GMximdex.ximlet(" + ID_imagen + ")@@@"; modificado por idemxime el 11/04/05 para poder recoger solo el numero de nodo con la misma funcion en init.php linea 270
	...
	if (navegador == "ie")
	{
		...
		{
			object = window.parent.campo_activo;
			window.parent.update_ximlet(object, arr["ID"]);
...
<!--
  for ( elem in window.dialogArguments )
  {
    ...
    case "url":
		...
		document.getElementById('previewImg').innerHTML = '<input type="Text"  class="cajag" ID="previewImgSrc" NAME="previewImgSrc" value="' + window.dialogArguments["url"] + '" BORDER=0 ALT="vista previa">';
     	ID_imagen = window.dialogArguments["url"];
		my_location = "../treeselector.php?contenttype=ximlet&targetid=" + ID_imagen;
...
document.write('<IFRAME application="yes" ID="appletdiv" scrolling="No" style="position:absolute;left:10px;top:27px;width:302px;height:308px;visibility:show" NAME="treeFrame" ID="treeFrame" WIDTH=302 HEIGHT=308 SRC="../treeselector.php?contenttype=ximlet&targetid=' + targetid + '"></IFRAME>');
</script>
