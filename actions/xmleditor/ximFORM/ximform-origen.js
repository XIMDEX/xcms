				...
				for (i = 0; i<servicio.length; i++){
					switch (servicio[i].tagName)
						...
						case "elemento_nombre":
							...
							if (servicio[i].getAttribute("valor")) {modifica_valores.elemento_valor.value = servicio[i].getAttribute("valor");}
							else modifica_valores.elemento_valor.value = "";
							break;
...
function inserta_valores(){
	if (!modifica_valores.campo.value){
		...
		nueva_opcion = new Option();
		nueva_opcion.value = modifica_valores.elemento_valor.value;
		nueva_opcion.text = modifica_valores.elemento_valor.value;
		form_final.elemento_valor.add(nueva_opcion);
		...
		modifica_valores.elemento_valor.value = "";
...
for (i=0;i<=el_objeto.length-1;i++){
	if (el_objeto.options[i].selected){
		...
		form_final.elemento_valor.options[i].selected = true;
	}
}
}
...
pos_seleccionado = -1;
	...
	modifica_valores.elemento_valor.value = form_final.elemento_valor.options[ultima_posicion].text;
	...
	form_final.elemento_valor.options[ultima_posicion] = null;
	...
	objeto = form_final.campo;
		...
		if (pos_seleccionado == -1){
			...
			if (modo=="sube") {
				...
				moveup(form_final.elemento_valor, pos_seleccionado);
				}
			if (modo=="baja"){
				...
				movedown(form_final.elemento_valor, pos_seleccionado);
				}
			}
...
function limpia_formfinal(){
	if (confirm ("A continuación se borrar&aacute;n todos los campos incluidos en el formulario \n &iquest;desea continuar?")){
		...
		limpia_combo(form_final.elemento_valor);
		}
	}
...
//	alert(xmlSer);
	...
	for (n = 0; n < longitud; n++){
		if (posiciones[n]){
			...
			newNode.setAttribute("elemento_valor", form_final.elemento_valor[n].value);
