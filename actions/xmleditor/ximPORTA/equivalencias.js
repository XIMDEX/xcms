...
/* 
	función que establece las propiedades de un enlace
	objeto: objeto creado para insertar el nuevo nodo XML 
	objetoHTML: objeto que contiene las propieades en HTML
*/
...
function prop_enlace(objeto, objetoHTML){
	...
	// compruebo si solo existe referenciador
	if (lacadena.length == 1){
		objeto.setAttribute("referenciador", lacadena[0]);
		}
	// si existe referenciador y/o enlace
	else if (lacadena.length == 2){
		// solo doy de alta la propiedad url si la dirección es http
		
		if(lacadena[1].indexOf("http")== 0 && lacadena[0] == "") { objeto.setAttribute("url", lacadena[1]);}
		else if (lacadena[0] != "" && lacadena[0].indexOf("http")== 0 ) { 
			objeto.setAttribute("url", lacadena[0] + "#" + lacadena[1]);
			lacadena[1] = "";
			}
		else{
			
			if (lacadena[1].indexOf("htm")!= -1) {alert(lacadena[1]);interno++;}
			if (lacadena[1].indexOf("pdf")!= -1) pdf++;
			if (lacadena[1].indexOf("doc")!= -1 || lacadena[1].indexOf("xls")!= -1) doc++;
			enlace_ext = false;
		}
		if (lacadena[0] == "" && lacadena[0].indexOf("http")!= 0 ) objeto.setAttribute("referenciador", lacadena[0]);
		else objeto.setAttribute("referenciador", lacadena[1]);
		enlace_enc = true;
	} 
	...
	if (enlace_ext) objeto.setAttribute(ximStyle, "enlacenegrita");
	else objeto.setAttribute(ximStyle, "ximporta");
}
