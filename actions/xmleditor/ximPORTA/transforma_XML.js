...
function muestra_todo(){
	if (document.getElementById('depuras') && depura){
		document.getElementById('depuras').style.visibility = "visible";
		document.getElementById('mitexto').value = "";
		document.getElementById('mitexto2').value = "";
...
function pega_codigo(){
		...
		if (!depura){
			if (enlace_enc){
				if (confirm ("Se han encontrado enlaces externos en el documento\n ¿desea darlos de alta en ximlink?")){
				
				enlaces.nodeid.value = Xnodeid;
				if (ximP.ximID.value != "") enlaces.nodeLINK.value = ximP.ximID.value;
				enlaces.xml.value = xmlDoc.xml;
				enlaces.submit();
				document.getElementById('tbContentElement').style.visibility = "hidden";
				document.getElementById('aceptar').style.visibility = "hidden";
				document.getElementById('ximio').style.visibility = "visible";
				reporta();
				}
			}
			else{
...
//					alert(el_nodo2.length);
					...
					else { el_nodo3 = el_nodo2(b).childNodes;}
...
var xmlDocT = new ActiveXObject("Msxml2.DOMDocument");
	if (nodo2.text == '' || nodo2.text == ' '){ if(nodo2.tagName){if (nodo2.tagName != "salto_parrafo"){ if (nodo2.tagName.indexOf("elemento") < 0 && nodo2.tagName != "enlace"){nodo1.removeChild(nodo2); num--;}}}}
	
	if (nodo1.tagName == "parrafo") {
		if(nodo1.text == '') {
				if (num<0) num = 0; nodo1.parentNode.removeChild(nodo1.parentNode.childNodes.item(num));
				}
		...
		// compruebo que las etiquetas "item" y "subtitulo" no tengan dentro parrafos o más subtítulos o item
	if (nodo1.tagName == "item" || nodo1.tagName == "subtitulo") {
		if (nodo1.childNodes.item(0).tagName){
			if ((nodo1.childNodes.item(0).tagName=="parrafo") || (nodo1.tagName == "subtitulo") || (nodo1.childNodes.item(0).tagName == "estilo" && nodo1.childNodes.item(0).getAttribute("clase") == "normal") || (nodo1.childNodes.item(0).tagName == "estilo" && nodo1.childNodes.item(0).getAttribute("clase") == "texto")){
		...
		// compruebo que la etiqueta subtitulo no tenga dentro enlaces: si los tiene sustituyo la etiqueta por enlaces
		if (nodo1.tagName=="subtitulo" && nodo1.childNodes){
			if (nodo1.childNodes.item(0).tagName){
	...
	if (nodo1.tagName == "enlace") {
		if(nodo1.parentNode.tagName != "parrafo" && nodo1.parentNode.tagName != "item")
			{
			newNode = xmlDocT.createNode(1, "parrafo", "");
			newNode.setAttribute(ximStyle, "normal");
			newNode2 = xmlDocT.createNode(1, "enlace", "");
...
function anadir_nodo(nodoHTML, xmlDoc, nodoXML, num){
	...
	if (e_xml =="fila") e_xml = e_xml + nodoHTML.childNodes.length;
	if (e_xml =="elemento") e_xml = e_xml + (num+1);
...
function pega_codigo_enlace(){
	var arr = new Array();
	arr["xml_content"] = top.ximio.enlaces.xml.value;
	window.returnValue = arr;
	window.close();
}
var texto = "";
function reporta(){
	...
	if (interno>0) texto = texto + "- Se ha(n) encontrado " + interno + " enlace(s) interno(s)\n";
	if (pdf>0) texto = texto + "- Se ha(n) encontrado " + pdf + " enlace(s) a documento(s) PDF\n";
	if (doc>0) texto = texto + "- Se ha(n) encontrado " + doc + " enlace(s) a documento(s) no estructurado(s) (.doc, xls, etc.) \n";
	if (texto != ""){
