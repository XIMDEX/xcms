...
class MPMProcessOutBool extends MPMProcess {
	...
	public function run(){
		...
		if ($this->callbackIsClass){
			if ($this->callbackIsPath){
				$function = $this->functionName;
				$args = $this->dataIn[$this->key];
				$ret = $this->factoryClass->$function($args);
			}else{
				//instanciate by refleccion
				$ret = $this->method->invoke(NULL,$this->dataIn[$this->key]);
			}
		}else{
			$ret = $this->method->invoke($this->dataIn[$this->key]);
		}
