...
// +----------------------------------------------------------------------+
//
// $Id: Server.php,v 1.65 2007/07/21 20:02:11 chregu Exp $
//
...
class HTTP_WebDAV_Server 
{
    ...
    function ServeRequest() 
    {
        ...
        // _urldecode transforma el path en funcion del cliente DAV y elimina caracteres no validos 
        ...
        // rfc2518, 5.2 Collection Resources
        // Cuando se solicita una coleccion esta debe contener el slash final,
        // en caso de que el cliente no lo haya incluido se debe redireccionar
        // al URL correcto.
        //
        // NOTE: Microsoft no respeta este apartado del RFC !!!
        //
        if ($this->path[strlen($this->path)-1] != '/' && XVFS::isDir($this->path)
        	&& ($this->_SERVER['REQUEST_METHOD'] == 'PROPFIND' || $this->_SERVER['REQUEST_METHOD'] == 'GET')
        	&& (strpos(strtolower($_SERVER['HTTP_USER_AGENT']), 'microsoft') === false)
        	) {
        	        	
        	$this->path = $_SERVER['REQUEST_URI'] . '/';        	
        	$this->http_status('301 Moved Permanently');
    ...
    */
    ...
    function _urldecode($path) 
    {
        ...
        if (preg_match("/[^A-Za-z0-9\_\-\.\/]+/", $path) > 0) {
			...
			$path2 = preg_replace('/&([a-zA-Z])(uml|acute|grave|circ|tilde);/', '$1', $path1);
			$path3 = html_entity_decode($path2);
        	$path = $path3;
