...
class Role extends Roles_ORM
{
	...
	function AddAction($actionID, $stateID=null, $idPipeline) {
		...
		$sql = sprintf("INSERT INTO RelRolesActions (IdRol,IdAction,IdState, IdPipeline)"
			. " VALUES (%s, %s, %s, %s)",
			$dbObj->sqlEscapeString($this->get('IdRole')),
			$dbObj->sqlEscapeString($actionID),
			$dbObj->sqlEscapeString($stateID),
			$dbObj->sqlEscapeString($idPipeline)
			);
	...
	function HasPermission($permissionID) {
		...
		return count($relRolesPermission->find('IdRel',
			'IdRole = %s AND IdPermission = %s',
			array($this->get('IdRole'), $permissionID),
			MONO)) > 0;
	}
	...
	function deleteAllRolesActions($idPipeline) {
       	$sql = sprintf("DELETE FROM RelRolesActions" .
        		" WHERE IdRol = %d AND IdPipeline = %d",
       			$this->get('IdRole'),
       			$idPipeline
       			);
	...
	{
		$dbObj = new DB();
		$query = sprintf("SELECT IdRel FROM RelRolesStates WHERE IdRole = %d AND  IdState = %d",
					$this->get('IdRole'), $transitionID);
		$dbObj->Query($query);
		if ($dbObj->numErr)
			$this->SetError(1);
	...
	*/
	function GetAllowedStates() {
		...
		$dbObj = new DB();
		$query = sprintf("SELECT IdState FROM RelRolesStates WHERE IdRole = %s AND IdState > 0",
				$dbObj->sqlEscapeString($this->get('IdRole')));
