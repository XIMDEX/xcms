...
/**
 * <p>SolrRepository class</p>
 * <p>Solr backend used to store node content</p>
 */
class SolrRepository
{
     ...
     * @param   array $options  (optional) Options for the Solr Client
     *
     */
    public function __construct($nodeID = null, $options = array())
    {
        $this->ClearError();
    ...
    public function GetContent($versionID = null, $subVersion = null)
    {
        ...
        $content = $this->fromSolr($versionID, $subVersion);
    ...
    /**
     * <p>Gets the content of the node for a specific version</p>
     * 
     * @param int $version
     * @param int $subversion
     * 
     * return sring The content of the node
     */
    private function fromSolr($version, $subversion) {
        $getContentQuery = new SolrQuery();
        $baseQuery = "nodeid:" . $this->nodeID;
        $baseQuery .= " AND version:" . $version . " AND subversion:" . $subversion;

        $getContentQuery->setQuery($baseQuery);
        $getContentQuery->addField("content");
        $getContentQuery->setRows(1);

        $queryResponse = $this->solrClient->query($getContentQuery);
        $resp = $queryResponse->getResponse();
        $docs = $resp["response"]["docs"];
        if (!is_array($docs)) {
            XMD_Log::warning('No se ha podido obtener el contenido');
            $this->SetError(3);
            return false;
        }
    ...
    public function SetContent($content, $versionID = NULL, $subVersion = NULL, $commitNode = NULL, $type = NULL)
    {
        ...
        if (!is_null($versionID) && !is_null($subVersion)) {
            ...
            $version = new Version($idVersion);
            $version->set('MimeType', $type);
            $this->toSolr($idVersion, $version, $content);
    ...
    public function AddVersion($jumpNewVersion = NULL, $comment = NULL, $content = NULL, $commitNode = NULL, $mimetype = NULL)
    {
        ...
        /* Adding node to Solr */
        $this->toSolr($IdVersion, $version, $newContent);
    ...
    /**
     * 
     * <p>Index a version to Solr</p>
     * @param int $IdVersion
     * @param Version $version
     * @param String $content
     * @param boolean $commit
     */
    private function toSolr($IdVersion, $version, $content, $commit = true) {
        $newDocument = new SolrInputDocument();

        $newDocument->addField("id", $IdVersion);
        $newDocument->addField("nodeid", $this->nodeID);
        ...
        $this->solrClient->addDocument($newDocument, false, 0);
        
        if ($commit || is_null($commit)) {
    ...
    public function DeleteSubversion($versionID, $subVersion = null)
    {
        ...
        $this->deleteFromSolr($versionID, $subVersion);
     ...
     * 
     * @param int $versionID
     * @param int $subversion (optional)
     */
    private function deleteFromSolr($versionID, $subversion) {
        if (is_null($subversion)) {
            $strQuery = "nodeid:".$this->nodeID." AND id:".$versionID;
            $versionToDelete = $versionID;
        } else {
            $strQuery = "nodeid:".$this->nodeID." AND version:".$versionID." AND subversion:".$subversion;
            $versionToDelete = $this->getVersionId($versionID, $subversion);
        }

        $solrUpdateQuery = $this->solrClient->deleteByQuery($strQuery);
        $this->performCommit();
    }
