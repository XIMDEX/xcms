...
/*
	 ...
	 primero:(optimiza en velocidad) sustituir los nombres por idNodeTypes (ahorrar� muchas consultas de selecci�n)
...
if($contentType) {
		...
		/// Los configuramos diciendo qu� tipos de nodo solo se muestran y qu� tipos de nodo, adem�s, pueden ser seleccionados.
	...
	//array_pop($pathList);
	// Quiz�s habr�a que pasar esta funci�n a la clase nodetype
	function getContainers($idNodeTypeArray) {
		//Condici�n de ejecuci�n de la funci�n
...
function PrintContent($nodeID, $contentType=null, $pathList=null, $targetNodeID=null, $filterType=null,$selectableList=null, $typeList=null, $userID, $idNodeType)
	...
	//*******************************************************
	// Comienzo de parte nueva a�adida por Jose Luis el 11.08.2004 para solo dejar ver los nodos asociados a tu grupo
	//*******************************************************
...
//	global $nelementos;
	...
	if(!$selectedNode->numErr) {
		...
		$childNode = null; // Declaraci�n de la variable

		if ($children) {
			$i=0;
			foreach($children as $childID) {
				//Esta condici�n tambi�n ha sido a�adida el 11.08.2004
...
function getNodelist($userID, $nodeID) {
	...
	if (!\Ximdex\Utils\Session::get("nodelist") or $nodeID==1)
	{
		...
		//vamos a�adiendo los padres de los nodos actuales
...
function getTypeAndSelectableList($contentType, &$typeList, &$selectableList) {
	...
	switch($contentType) {
		...
		//Parte a�adida para la acci�n asociar pvd a rol. Se pretende presentar las plantillas vista en el �rbol.
		case  'pvds': /* ##################### pvds ############################ */
