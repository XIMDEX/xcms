...
function presenta_images(){ 
 	if(http1.readyState == 4){			    
	   ...
	   //Second table with the image batch list
...
function changeScaleR(obj){ //Changing scale for RadioButtons
...
function wrapp_fila(fila){
     //Table of one row with two columns inside a column in a receptor table row.
...
function img_nav1(obj,valor){
     ...
     //Accessing the table body in which the vieser is going to be added in a line
     var recep = document.getElementById("receptor");
		 var cuerpo = recep.firstChild;
     var fila = document.createElement("tr");
		 var colspan="colspan='2'"
		 ...
		 add_column2(fila,colspan); 
		 //transforms a row of two columns in a column though a table
