...
X.actionLoaded(function (event, fn, params){
	...
	function refresh_frame_list(data, initial_state) {
		...
		for (var portal_version in data) {
			if(initial_state) extended[portal_version] = false;
			var acum_progress = 0;
			var nodename = '';
                        var batchid = 0;
			...
			$(data[portal_version]).each(function (key, item) {
			...
			if(extended[portal_version] === false) $('.portal_version_' + portal_version, content).hide();
	...
	function get_frame_list(initial_state, batch_id) {
		$.get(params.actionView.url, {method: 'getFrameList', batchid: batch_id}, function(data) {
			if(data) {
				var frame_list = eval("(" + data + ")");
				refresh_frame_list(frame_list, initial_state);
			}
		}.bind(this));
	}
