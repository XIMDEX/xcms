...
<div class="action_content">
    {if $linked_ximlets and count($linked_ximlets) > 0 }
		<form method="post" id="delete_rel" class="delete_rel" action="{$action_delete}">
		    ...
		    <fieldset>
				...
				<div class="copy_options">
				    {foreach from=$linked_ximlets item=ximlet_info}
				        <div>
					       <input type="checkbox" id="{$id_node}_idximlet_{$ximlet_info.idximlet}" name="idximlet[]" 
					               value="{$ximlet_info.idximlet}" />
		    ...
		    <fieldset>
                <input type="checkbox" name="recursive" id="{$id_node}_recursive_delete" /> <label 
                        for="{$id_node}_recursive_delete"> {t}Disassociate recursively{/t}.</label>
    ...
    {/if}
    {if $linkable_ximlets and count($linkable_ximlets) > 0 }
		<form id="create_rel" class="create_rel" action="{$action_create}" method="post">	
			<fieldset>
				...
				<div class="copy_options">
					...
					{foreach from=$linkable_ximlets item=ximlet}
                        <div>
                            <input type="checkbox" name="idximlet[]" id="{$id_node}_idximletavailable_{$ximlet.idximlet}" 
                                    value="{$ximlet.idximlet}" />
			...
			<fieldset class="buttons-form">
				{button label="Associate" class='validate btn main_action'}
			</fieldset>
		</form>
	{else}
