...
class Action_fileupload extends ActionAbstract
{
    ...
    function fileupload()
    {
        ...
        $messages = $this->uploadCommonFile($idNode);
        if ($messages === false) {
            return; // makes a redirect;
        }
        if (! $messages->count(MSG_TYPE_ERROR)) {
            $node = new Node($idNode);
            if ($node->nodeType->get('IsFolder')) {
    ...
    private function uploadCommonFile(int $idNode)
    {
        ...
        if (! $nodeTypeName) {
            ...
            $types = $rntmt->find('distinct extension', 'idNodeType in (%s)', array(
                $inSearch
            ), MONO, false);
            if (empty($types)) {
                $this->messages->add(_('No type of file which can be inserted in this folder has been found, contact with your administrator')
                    , MSG_TYPE_ERROR);
            } else {
        ...
        if ($node->nodeType->get('IsFolder')) {
            ...
            if ($result > 0) {
                ...
                if ($node->nodeType->get('Name') == 'TemplatesRootFolder')
                    $node->class->updateChooseTemplates();
            } else {
        ...
        $tmpFolder = XIMDEX_ROOT_PATH . App::getValue('TempRoot') . '/uploaded_files/';
        $tmpFile = FsUtils::getUniqueFile($tmpFolder);
        move_uploaded_file($filePath, $tmpFolder . $tmpFile);
        $this->redirectTo('confirm', null, array(
            'tmp_file' => $tmpFile,
            'tmp_name' => $fileName,
            'id_node' => $idNode,
    ...
    public function confirm()
    {
        ...
        if (strcmp($nodeTypeName, 'ImageFile') == 0) {
            ...
            $this->messages->add(sprintf(_('File content %s is going to be replaced with %s'), $node->get('Name'), $tmpName)
                , MSG_TYPE_WARNING);
        }
