    ...
    <div class="action_content">
	        ...
	        {if ($new_groups)}
	            <div class="small-12 columns">
	                <fieldset class="buttons-form">
	                    {button label="Add group" onclick="call_submit('addgroupnode');" class="validate btn main_action" }
	                    {* message="Are you sure you want to perform this association?" *}
	                </fieldset>
	            </div>
	        {/if}
	    </div>
	    <div class="row tarjeta">
	            ...
	            <table class="table table-striped">
		            ...
		            {foreach from=$all_groups item=groupInfo}
		                <tr>
			                ...
			                <td class='filaclara'>{$groupInfo.Name}</td>
			                <td class='filaclara'>
			                    ...
			                    {* <select name='idRole[]' class='caja'>
			                        <option value=''>{t}None{/t}</option>
			                        {foreach from=$roles item=rol_info}
			                            <option value="{$rol_info.IdRole}" {if $groupInfo.IdRoleOnNode==$ rol_info.IdRole} 
			                                    selected="selected" {/if}>{$rol_info.Name}</option>
			                        {/foreach}
			                    </select> *}
	        ...
	        {if ($all_groups)}
	            <div class="small-12 columns">
	                <fieldset class="buttons-form">
	                    {button label="Delete subscriptions" class="validate btn main_action" onclick="call_submit('deletegroupnode')" 
	                            message="Are you sure you want to delete selected subscriptions?"}
	                </fieldset>
	            </div>
	        {/if}
	    </div>
