...
X.actionLoaded(function (event, fn, params)
{
    ...
    function show_local_fields(label)
    {
        ...
        if (label === 'LOCAL') {
            ...
            if (label === 'API') {
                fn('#labelDirectorio').text(_('Endpoint'));
                fn('.login').addClass('hidden');
                fn('.password').addClass('hidden');
                fn('.token').removeClass('hidden');
            }
            else {
                fn('#labelUrl').text(_('Remote URL'));
                fn('#labelDirectorio').text(_('Remote directory'));
                fn('.login').removeClass('hidden');
                fn('.password').removeClass('hidden');
                fn('.token').addClass('hidden');
            }
