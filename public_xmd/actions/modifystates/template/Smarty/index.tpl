...
<form method="post" id="msu_form" ng-controller="XModifyStatesCtrl" ng-init="idNode={$idNode};">
    ...
    <div class="action_content">
        <div class="row tarjeta">
            ...
            <ul ui-sortable="sortableOptions" class="sortable" ng-model="all_status_info" 
                ...
                <li ng-repeat="status in all_status_info" ng-class="{literal}{sortable: $middle, first: $first, last: $last}{/literal}">
                    <label for="id_#/status.id/#" class="status li_central label_general" 
                        style="margin-left: 20px !important;"> {t}Status{/t} #/$index+1/#: </label> 
                    <input ng-disabled="!$middle" placeholder="{t}Status name{/t}" type="text" id="id_#/status.id/#" class="name input_general" 
                            ng-model="status.name" title="Status name" />
                    <input ng-disabled="!$middle" placeholder="{t}Description{/t}" type="text" class="description input_general" 
                            ng-model="status.description" title="Status description" />
                    <select class="action" ng-disabled="!$middle" placeholder="{t}Action{/t}" ng-model="status.action" 
                            title="Action to do before the status has been reached">
            ...
            <div class=" {if $is_workflow_master == true}disabled{/if}">
                ...
                <div class="small-10 columns">
                    <div class="input-select2">
                        <select ng-model="nodetype" ng-options="nt.id as nt.name for nt in nodetype_list" class="cajag form-control" 
                                {if $is_workflow_master==true}disabled{/if}>
