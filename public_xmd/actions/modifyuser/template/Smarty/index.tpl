...
<form method="post" id="mu_action" action="{$action_url}">
	...
	<div class="action_content">
		<div class="row tarjeta">
			...
			<div class="small-6 columns">
				<div class="input">
				   ...
				   <input type="password" name='password_' id="password_" 
                            class="input_general full_size validable js_val_min__6" 
			...
			<div class="small-6 columns">
				<div class="input">
				    ...
				    <input type="password" name='password_repeated' id="password_repeated" 
                           class="input_general full_size validable" placeholder="{t}Repeat password{/t}" 
                           tabindex="4">
				</div>
		    </div>
		    <div class="small-6 columns">
			    <div class="input-select" style="margin-bottom:15px !important;">
				    ...
				    <select {if ! $canModifyUserGroup} disabled="disabled" {/if} name="generalrole" id="generalrole" class="full_size" 
                        ...
                        {foreach from = $roles item = role}
                            <option value="{$role.IdRole}" {if ($role.IdRole == $general_role)} selected="selected" {/if}>{$role.Name}</option>
                        {/foreach}
				    </select>
			    </div>
			</div>
		    <div class="small-6 columns">
			    <div class="input-select" style="margin-bottom:15px!important;">
				   <label for="locale" class="label_title label_general">{t}Interface language{/t} *</label>
