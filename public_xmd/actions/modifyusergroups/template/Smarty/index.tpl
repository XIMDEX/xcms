            ...
            <div class="associate-group">
                ...
                <div class="small-12 columns">
                    <div class="alert alert-info" ng-hide="filtered_groups.length>0">
                        <strong>Info!</strong>
                        <p>{t}There are not{/t} <span ng-if="user_groups_with_role.length>0">{t}more{/t} </span>
                        {t}available groups to be associated with the user{/t}</p>
                    </div>
                </div>
                <div class="small-12 columns">
                    <fieldset class="buttons-form">
                        <button type="button" id="" ng-click="addGroup()" 
                                class="btn ui-state-default ui-corner-all button submit-button ladda-button main_action" 
                                data-style="slide-up" data-size="xs" tabindex=""><span class="ladda-label">{t}Add{/t}</span></button>
                    </fieldset>
                </div>
            </div>
        </div>
                ...
                <div ng-repeat="user_group_info in user_groups_with_role" class="row-item icon">
                    ...
                    <div style="bottom: -70px;" class="buttons-form row-item-actions col1-3">
                        ...
                        <span ng-if="user_group_info.IdGroup != '101'">
                            <button type="button" class="delete-btn icon btn-unlabel-rounded-delete" ng-click="openDeleteModal($index)">
