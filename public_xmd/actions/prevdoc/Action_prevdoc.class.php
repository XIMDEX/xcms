...
class Action_prevdoc extends ActionAbstract
{
    public function index()
    {
        ...
        if (! $content) {
            $content = $structuredDocument->GetContent($this->request->getParam('version'), $this->request->getParam('sub_version'));
        ...
        if ($content === false) {
            ...
            $prevUrl = $queryManager->getPage() . $queryManager->buildWith(array(
                'method' => 'prevdoc',
                'hash' => $hash
            ));
            if ($json == 'json') {
