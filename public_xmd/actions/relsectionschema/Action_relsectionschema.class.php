...
class Action_relsectionschema extends ActionAbstract {
	...
	function set_property() {
		...
		if ($selected == 'all') {
		    $selected = array(\Ximdex\Services\NodeType::VISUAL_TEMPLATE, \Ximdex\Services\NodeType::RNG_VISUAL_TEMPLATE);
		}
		$node = new Node($idNode);
		...
		$values = array(
				...
				'id_node' => $idNode,
				"nodeURL" => App::getValue('UrlRoot')."/public_xmd/?actionid=$idAction&nodeid={$idNode}",
				);
