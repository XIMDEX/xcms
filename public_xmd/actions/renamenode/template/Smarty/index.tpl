...
<form method="post" name="ren_form" id="ren_form" action="{$action_url}">
	...
	<div class="action_content">
		<div class="row tarjeta">
			...
			{if $id_nodetype == 5078}
				<div class="small-12 columns">
				    <input type="radio" name="schema_type" value="generic_schema"{if $schema_type == 'generic_schema' || $schema_type == ''} 
				            checked="checked"{/if} id="generic-scheme_{$id_node}" class="hidden-focus">
				    <label for="generic-scheme_{$id_node}" class="icon radio-label">{t}Generic schema{/t}</label>
				</div>
				<div class="small-12 columns">
				    <input type="radio" name="schema_type" value="metadata_schema"{if $schema_type == 'metadata_schema'} 
				            checked="checked"{/if} id="metadata-scheme_{$id_node}" class="hidden-focus">
	            ...
	            <div class="small-12 columns">
			        ...
			        {foreach from=$all_languages item=language_info}
				        <div class="languages-section">
                            <input type="checkbox" name="languages[]" id="lang_{$language_info.Name}" value="{$language_info.IdLanguage}" 
                                    class= "hidden-focus" />
					        <label for="lang_{$language_info.Name}" class="icon checkbox-label">{$language_info.Name}</label>
					        <input type='text' name="language[{$language_info.IdLanguage}]" value="{$language_info.alias}" 
					        class="alternative-name" placeholder="Nombre alternativo para migas de pan y rutas">
				        </div>
			        {/foreach}
		      </div>
	       {/if}
