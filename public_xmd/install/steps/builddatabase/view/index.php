...
<form method="post"  ng-controller="InstallDatabaseController" ng-cloak name="formDataBase" ui-keypress="{13:'sendForm($event)'}" 
	...
	<div ng-show="!installed">
		<p>Now it's time to install a MySQL or MariaDB database. A priviledged user (i.e. root) will be required to create 
		Ximdex's database.</p>
		...
		<button ng-hide="dbErrors" class="launch_ximdex action_launcher ladda-button" ui-ladda="loading" data-style="slide-up" 
				xim-state="loading" ng-click="sendForm()">Create Database</button>
		<button ng-show="dbErrors" class="launch_ximdex action_launcher ladda-button" ui-ladda="loadingOverwrite" 
				data-style="slide-up" xim-state="loadingOverwrite" ng-click="sendForm()">Overwrite database</button>
	</div>
	<div ng-show="installed">
		<p>
			Once the database is created, it is a good practice to use an <em>unpriviledged user</em> to access it. 
			Set the name and password for the database user accesing the Ximdex's database. 
