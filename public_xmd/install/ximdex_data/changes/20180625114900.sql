...
ALTER TABLE `Batchs` ADD CONSTRAINT `Batchs_Nodes` FOREIGN KEY (`IdNodeGenerator`) 
    REFERENCES `Nodes`(`IdNode`) ON DELETE SET NULL ON UPDATE CASCADE;
ALTER TABLE `Batchs` ADD CONSTRAINT `Batchs_PortalVersions` FOREIGN KEY (`IdPortalVersion`) 
    REFERENCES `PortalVersions`(`id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE `Batchs` ADD CONSTRAINT `Batchs_Users` FOREIGN KEY (`UserId`) 
...
ALTER TABLE `Pumpers` ADD CONSTRAINT `Pumpers_Servers` FOREIGN KEY (`IdServer`) 
...
ALTER TABLE `Nodes` ADD CONSTRAINT `Nodes_nodes` FOREIGN KEY (`IdParent`) 
