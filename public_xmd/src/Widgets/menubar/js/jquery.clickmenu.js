	...
	$.fn.clickMenu = function(options) 
	{
		...
		var mainHoverIn = function(e)
		{
			...
			for (var i = 0; i < lis.length; i++)
			{
				...
				{
					$(lis[i]).removeClass('hover ui-state-hover');
				}
			}
			$(this).addClass('hover ui-state-hover');
			if ( shown )
			{
		...
		var hoverIn = function(li, delay)
		{
			...
			for ( ; pNode; pNode = pNode.parentNode ) 
			{
				if ( pNode.nodeType == 1 && pNode.nodeName.toUpperCase() == 'DIV' )
				{
					if (pNode.timer)
					{
						...
						pNode.timer = null;
						$(pNode.parentNode).addClass('hover ui-state-hover');
					}
				}
			}
			//highlight the current element
			$(li).addClass('hover ui-state-hover');
			...
			$(li.parentNode.getElementsByTagName('DIV')).each(function(){
				...
				{
					hideDIV(this, delay);
					$(this.parentNode).removeClass('hover ui-state-hover');
				}
			});
		...
		var liHoverOut = function(e)
		{
			...
			if ( !div )
			{
				$(this).removeClass('hover ui-state-hover');
			}
			else 
			{
				if ( !div.isVisible )
		...
		var clean = function()
		{
			...
			$('ul.clickMenu li').removeClass('hover ui-state-hover');
			//remove events
