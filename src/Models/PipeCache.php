...
class PipeCache extends PipeCachesOrm
{
    ...
    function load($idVersion, $idTransition, $args = NULL, $depth = 0)
    {
        ...
        else
        {
            ...
            if (isset($args['CONTENT']))
            {
                ...
                {
                    $pointer = XIMDEX_ROOT_PATH . TMP_FOLDER . "preview_" . $_GET["nodeid"] . "_"
                            . FsUtils::getUniqueFile(XIMDEX_ROOT_PATH . TMP_FOLDER);
                }
                else
                {
            ...
            Logger::info('PipeCache: Obtained file ' . $pointer . ' to generate the pipeline transition ' . $idTransition . ' for version ' 
                    . $idVersion);
    ...
    public function store($idVersion, $idTransition, & $contentFile, $args)
    {
        ...
        if (empty($cacheFile))
        {
            ...
            {
                Logger::error('PipeCache: An error has ocurred while storing the cache file ' . $contentFile . ' in the target ' . XIMDEX_ROOT_PATH 
                        . CACHE_FOLDER . $cacheFile);
                return false;
            }
